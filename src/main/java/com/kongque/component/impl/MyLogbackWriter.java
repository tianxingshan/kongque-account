 ...
 * @author yuehui
 *
 * @date: 2018年9月10日 下午2:37:56
 */
public class MyLogbackWriter<E> extends FileAppender<E> {
	...
	private boolean checkForFileAndPatternCollisions() {
		try {
			if (triggeringPolicy instanceof RollingPolicyBase) {
				...
				Field f = RollingPolicyBase.class.getDeclaredField("fileNamePattern");
				f.setAccessible(true);
				final FileNamePattern fileNamePattern = (FileNamePattern) f.get(base);// base.fileNamePattern;
				// no use checking if either fileName or fileNamePattern are
				// null
	...
	private void setPermission(String file) {
		...
		if (f.exists()) {
			...
			try {
				...
				//文件
				Files.setPosixFilePermissions(Paths.get(tmp), set);
				int p = tmp.indexOf("projectLogs");
				if (p > -1) {
					while (tmp.indexOf(File.separator, p) > -1) {
						int t = tmp.indexOf(File.separator, p);
						String s=tmp.substring(0, t+1);
						//父级目录
						Files.setPosixFilePermissions(Paths.get(s), set);
						p = t+1;
					}
				}
			} catch (IOException e) {
